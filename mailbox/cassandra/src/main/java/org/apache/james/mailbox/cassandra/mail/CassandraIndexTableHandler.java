...
public class CassandraIndexTableHandler {
    ...
    public Mono<Void> updateIndexOnDelete(CassandraId mailboxId, Collection<MessageMetaData> metaData) {
        return Flux.mergeDelayError(Queues.XS_BUFFER_SIZE,
                updateFirstUnseenOnDeleteWithMetadata(mailboxId, Flux.fromIterable(metaData)),
                updateRecentOnDeleteWithMetadata(mailboxId, Flux.fromIterable(metaData)),
                updateDeletedMessageProjectionOnDeleteWithMetadata(mailboxId, Flux.fromIterable(metaData)),
    ...
    public Mono<Void> updateIndexOnDeleteComposedId(CassandraId mailboxId, Collection<ComposedMessageIdWithMetaData> metaData) {
        return Flux.mergeDelayError(Queues.XS_BUFFER_SIZE,
                updateFirstUnseenOnDelete(mailboxId, Flux.fromIterable(metaData)),
                updateRecentOnDeleteWithComposeId(mailboxId, Flux.fromIterable(metaData)),
                updateDeletedMessageProjectionOnDelete(mailboxId, Flux.fromIterable(metaData)),
    ...
    private Mono<Void> updateRecentOnDeleteWithMetadata(CassandraId mailboxId, Flux<MessageMetaData> metaDataFlux) {
        return mailboxRecentDAO.removeFromRecent(mailboxId, metaDataFlux.filter(metaData -> metaData.getFlags().contains(Flags.Flag.RECENT))
            .map(MessageMetaData::getUid));
    }

    private Mono<Void> updateRecentOnDeleteWithComposeId(CassandraId mailboxId, Flux<ComposedMessageIdWithMetaData> composedMessageIdWithMetaDataFlux) {
        return mailboxRecentDAO.removeFromRecent(mailboxId, composedMessageIdWithMetaDataFlux.filter(composedId -> composedId.getFlags().contains(Flags.Flag.RECENT))
    ...
    private Mono<Void> updateDeletedMessageProjectionOnDelete(CassandraId mailboxId, Flux<ComposedMessageIdWithMetaData> flux) {
        return deletedMessageDAO.removeDeleted(mailboxId, flux.filter(composedId -> composedId.getFlags().contains(Flags.Flag.DELETED))
            .map(composedId -> composedId.getComposedMessageId().getUid()));
    }

    private Mono<Void> updateDeletedMessageProjectionOnDeleteWithMetadata(CassandraId mailboxId, Flux<MessageMetaData> flux) {
        return deletedMessageDAO.removeDeleted(mailboxId, flux.filter(metaData -> metaData.getFlags().contains(Flags.Flag.DELETED))
            .map(MessageMetaData::getUid));
    }
    ...
    public Mono<Void> updateIndexOnAdd(Collection<MailboxMessage> messages, CassandraId mailboxId) {
        ...
        return Flux.mergeDelayError(Queues.XS_BUFFER_SIZE,
                checkDeletedOnAdd(mailboxId, Flux.fromIterable(messages)),
                updateFirstUnseenOnAdd(mailboxId, Flux.fromIterable(messages)),
                addRecentOnSave(mailboxId, Flux.fromIterable(messages)),
                incrementCountersOnSave(mailboxId, flags),
    ...
    private Mono<Void> updateDeletedOnFlagsUpdate(CassandraId mailboxId, List<UpdatedFlags> updatedFlags) {
        Flux<MessageUid> addDeletedUids = Flux.fromIterable(updatedFlags)
            .filter(flag -> flag.isModifiedToSet(Flags.Flag.DELETED))
            .map(UpdatedFlags::getUid);
        Flux<MessageUid> removeDeletedUids = Flux.fromIterable(updatedFlags)
            .filter(flag -> flag.isModifiedToUnset(Flags.Flag.DELETED))
            .map(UpdatedFlags::getUid);

        return Flux.mergeDelayError(Queues.XS_BUFFER_SIZE,
            deletedMessageDAO.addDeleted(mailboxId, addDeletedUids),
            deletedMessageDAO.removeDeleted(mailboxId, removeDeletedUids))
            .then();
    }
    ...
    private Mono<Void> addRecentOnSave(CassandraId mailboxId, Flux<MailboxMessage> messages) {
        return mailboxRecentDAO.addToRecent(mailboxId, messages.filter(message -> message.createFlags().contains(Flags.Flag.RECENT))
    ...
    private Mono<Void> manageRecentOnFlagsUpdate(CassandraId mailboxId, List<UpdatedFlags> updatedFlags) {
        Flux<MessageUid> addRecentUids = Flux.fromIterable(updatedFlags)
            .filter(flag -> flag.isModifiedToSet(Flags.Flag.RECENT))
            .map(UpdatedFlags::getUid);
        Flux<MessageUid> removeRecentUids = Flux.fromIterable(updatedFlags)
            .filter(flag -> flag.isModifiedToUnset(Flags.Flag.RECENT))
            .map(UpdatedFlags::getUid);

        return Flux.mergeDelayError(Queues.XS_BUFFER_SIZE,
                mailboxRecentDAO.removeFromRecent(mailboxId, removeRecentUids),
                mailboxRecentDAO.addToRecent(mailboxId, addRecentUids))
            .then();
    }
    ...
    private Mono<Void> updateFirstUnseenOnAdd(CassandraId mailboxId, Flux<MailboxMessage> mailboxMessageFlux) {
        return firstUnseenDAO.addUnread(mailboxId, mailboxMessageFlux.filter(mailboxMessage -> !mailboxMessage.createFlags().contains(Flags.Flag.SEEN))
            .map(MailboxMessage::getUid));
    }
    ...
    private Mono<Void> checkDeletedOnAdd(CassandraId mailboxId, Flux<MailboxMessage> mailboxMessages) {
        return deletedMessageDAO.addDeleted(mailboxId, mailboxMessages.filter(message -> message.createFlags().contains(Flags.Flag.DELETED))
            .map(MailboxMessage::getUid));
    }
    ...
    private Mono<Void> updateFirstUnseenOnDeleteWithMetadata(CassandraId mailboxId, Flux<MessageMetaData> metaDataFlux) {
        return firstUnseenDAO.removeUnread(mailboxId, metaDataFlux.filter(metaData -> !metaData.getFlags().contains(Flags.Flag.SEEN))
            .map(MessageMetaData::getUid));
    }

    private Mono<Void> updateFirstUnseenOnDelete(CassandraId mailboxId, Flux<ComposedMessageIdWithMetaData> composedMessageIdWithMetaDataFlux) {
        return firstUnseenDAO.removeUnread(mailboxId, composedMessageIdWithMetaDataFlux.filter(composeId -> !composeId.getFlags().contains(Flags.Flag.SEEN))
    ...
    private Mono<Void> updateFirstUnseenOnFlagsUpdate(CassandraId mailboxId, List<UpdatedFlags> updatedFlags) {
        Flux<MessageUid> addUnreadUids = Flux.fromIterable(updatedFlags)
            .filter(flag -> flag.isModifiedToUnset(Flags.Flag.SEEN))
            .map(UpdatedFlags::getUid);
        Flux<MessageUid> removeUnreadUids = Flux.fromIterable(updatedFlags)
            .filter(flag -> flag.isModifiedToSet(Flags.Flag.SEEN))
            .map(UpdatedFlags::getUid);

        return Flux.mergeDelayError(Queues.XS_BUFFER_SIZE,
                firstUnseenDAO.addUnread(mailboxId, addUnreadUids),
                firstUnseenDAO.removeUnread(mailboxId, removeUnreadUids))
            .then();
    }
}
